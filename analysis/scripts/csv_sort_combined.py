    ...
    try:
        with open(sleep_stage_file, 'r') as f:
            ...
            for row in reader:
                ...
                if stage_type not in {'REM', 'DEEP'}: continue
                ...
                date = end.date()
                if date not in sleep_stages: sleep_stages[date] = {'rem_minutes': 0.0, 'deep_minutes': 0.0}
                duration_minutes = (end - start).total_seconds() / 60
                if stage_type == 'REM': sleep_stages[date]['rem_minutes'] += duration_minutes
                else: sleep_stages[date]['deep_minutes'] += duration_minutes
    except (FileNotFoundError, KeyError, ValueError): pass
    return sleep_stages
...
def read_form_data(form_file, person):
    ...
    try:
        with open(form_file, 'r') as f:
            reader = csv.DictReader(f)
            for row in reader:
                if row['Naam'].strip().lower() != person: continue
...
def process_person(person):
    ...
    if person == 'lucas': calories_file = f"/Users/robinoffringa/Desktop/De-Wetenschappelijke-Cyclus/raw_data/takeout_{person}/Fitbit/Physical Activity_GoogleData/active_energy_burned_2026-05-01.csv"

    heart_rate_files = [f"heart_rate_2026-04-06.csv"] + [f"heart_rate_2026-05-{i:02d}.csv" for i in range(6, 22)]
    ...
    for timestamp, bpm in awake_heart_rate:
        date = timestamp.date()
        if date not in daily_data: daily_data[date] = {'heart_rates': [], 'min_hr': bpm, 'max_hr': bpm}
    ...
    for date in sorted(daily_data.keys()):
        if date in daily_steps:
            ...
            duration = calculate_sleep_duration(sleep_times.get(date, {}).get('start', ''), sleep_times.get(date, {}).get('end', ''))
            raw_score = sleep_scores.get(date, '')
            corr_score = calculate_corrected_score(raw_score, duration)
                
            row_data = {
                'naam': person.capitalize(), 'date': date, 'average_heart_rate': round(avg_hr, 2),
                'min_heart_rate': daily_data[date]['min_hr'], 'max_heart_rate': daily_data[date]['max_hr'],
                'start': sleep_times.get(date, {}).get('start', ''), 'end': sleep_times.get(date, {}).get('end', ''),
                ...
                'sleep_score': raw_score, 'sleep_duration_hours': duration, 'corrected_sleep_score': corr_score,
                'calories_burned': round(daily_calories.get(date, 0), 2), 'total_steps': daily_steps[date],
            ...
            if date in sleep_scores: person_sleep_score_results.append(row_data)
...
output_file_raw = "/Users/robinoffringa/Desktop/De-Wetenschappelijke-Cyclus/analysis/data/combined_raw/combined_raw.csv"
sleep_score_output_file = "/Users/robinoffringa/Desktop/De-Wetenschappelijke-Cyclus/analysis/data/combined_filetered/combined_data_filtered.csv"
fieldnames = ['naam', 'date', 'average_heart_rate', 'min_heart_rate', 'max_heart_rate', 'start', 'end', 'rem_sleep_minutes', 'deep_sleep_minutes', 'sleep_score', 'sleep_duration_hours', 'corrected_sleep_score', 'calories_burned', 'total_steps', 'phone_last_used', 'last_food_time', 'last_food']
