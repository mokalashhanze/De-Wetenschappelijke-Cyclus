...
In het model zonder `naam` als factor is `resting_heart_rate` significant gerelateerd aan `average_sleep_heart_rate` (p < 0.05). Ook `last_food = snack` komt naar voren (p ≈ 0.035), maar die p‑waarde zit aan de rand en is minder betrouwbaar. Zodra we echter `naam` als factor toevoegen om voor individuele verschillen te corrigeren, verdwijnt die significantie. Dat betekent: op groepsniveau zien we wel een verband (lagere rusthartslag → lagere slaaphartslag), maar met deze dataset kun je niet betrouwbaar concluderen dat dat ook altijd voor één specifieke persoon geldt. Bovendien is de power laag (≈ 0.23); om het geobserveerde zwakke effect betrouwbaar te detecteren is naar schatting ≈ 460 dagen nodig.
